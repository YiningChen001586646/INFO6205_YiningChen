...
public class UF_HWQUPC implements UF {
    ...
    public int UnionFind (int n,boolean pass) {
    	UF_HWQUPC uf =new UF_HWQUPC(n,pass);
    	int pairs=0;
    	while(uf.components()!=1) {
    	int p= ThreadLocalRandom.current().nextInt(0, n-1 + 1);
    	int q= ThreadLocalRandom.current().nextInt(0, n-1 + 1);
    		if (!uf.connected(p, q)){
    			uf.union(p, q);
    			}
    		pairs++;
    ...
    public static void main(String[] args){
    	UF_HWQUPC uf =new UF_HWQUPC(10);
    	uf.UnionFind(10,true);
    	
	/*	for (int i=2;i<1000;i*=2) {
    		int avg=0;
         	int m=0;
         	for (int j=0;j<100;j++) {
    			UF_HWQUPC uf =new UF_HWQUPC(i*10);
    			avg+=uf.UnionFind(i*10,true);
    		}
    		m=avg/100;
